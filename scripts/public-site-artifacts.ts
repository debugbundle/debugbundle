...
function buildLlmsTxt(): string {
  return ensureTrailingNewline(
    [
      ...
      '',
      'Documentation',
      `- Core concepts: ${toAbsoluteUrl('/docs/v1/overview/')}`,
      `- API reference: ${toAbsoluteUrl('/docs/v1/api/')}`,
      `- CLI reference: ${toAbsoluteUrl('/docs/v1/cli/')}`,
      `- MCP reference: ${toAbsoluteUrl('/docs/v1/mcp/')}`,
      `- Webhook events: ${toAbsoluteUrl('/docs/v1/webhooks/')}`,
