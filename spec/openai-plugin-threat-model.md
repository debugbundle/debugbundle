...
The exact data-free catalog also passes MCP Inspector 2.5.0 strict validation locally with no schema errors or portability warnings. The candidate makes every advertised local JSON Schema reference self-contained and serializes nullable type unions through equivalent `anyOf` branches. This changes only the advertised schema representation; the normative schemas and runtime validation semantics remain unchanged. It is not production evidence until a separately authorized deployment promotes the candidate.
