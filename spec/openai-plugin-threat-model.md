...
| Monitoring misses OpenAI failure        | Metadata-only handled DebugBundle incidents for alert-worthy MCP/OAuth/reviewer signals; normal auth discovery excluded; independent free-tier UptimeRobot MCP readiness; existing AWS baseline retained | Sanitized capture tests, deployed dogfooding incident proof, external readiness/TLS monitor evidence |
...
Mitigation order is reduce bounds, tighten expensive-tool limits, lower concurrency, disable optional improvement tools, limit availability, then disable MCP. Use the no-deploy Caddy gate for an urgent MCP-only incident. UptimeRobot provides independent readiness/TLS detection only and never triggers shared-host recovery. A larger host, separate instance, paid identity provider, or paid monitor requires owner approval.
...
- fail-safe metadata-only DebugBundle incident reporting for alert-worthy MCP/OAuth/reviewer signals, with normal unauthenticated discovery excluded from incidents and the AWS baseline installer prevented from creating superseded OpenAI custom metrics.

The exact data-free catalog also passes MCP Inspector 2.5.0 strict validation locally with no schema errors or portability warnings. The candidate makes every advertised local JSON Schema reference self-contained and serializes nullable type unions through equivalent `anyOf` branches. This changes only the advertised schema representation; the normative schemas and runtime validation semantics remain unchanged. Hosted run `33907119277` deployed the correction on the existing shared Lightsail runtime at API/worker digest `sha256:349c98954955643f3f14cb1070623bbf7c25d3e6b1402d35214bca9479d3fdb7`; its migration ledger, preserved OAuth/MCP/reviewer/Caddy configuration, and independent public API/MCP/OAuth boundary checks pass. Authenticated OpenAI catalog refresh remains separate live-client evidence.

Production evidence now includes the additive migration, same-Lightsail immutable image deployment, managed Caddy and DNS/TLS activation, OAuth/reviewer/MCP configuration, OAuth and MCP metadata, the bounded live ChatGPT discovery challenge, successful owner reconnect, the captured non-secret Developer Mode connection mapping, passing project/service plus incident/context/existing-artifact ChatGPT cases, a passing fresh-thread Codex aggregate-analytics case, and a passing primary endpoint-health case with sanitized URL, recent-result, daily-rollup, and bounded linked-incident evidence. The endpoint-health run also exposed bounded older-result pagination failures. At 10:55 UTC on 2026-09-04, the owner confirmed that the exact returned cursor and unchanged project/check/lookback/limit still failed on page two. A red public-handler regression reproduced the cause: strict input validation compared serialized objects and therefore rejected a valid client object whose `cursor` member followed `limit`. Structural equality now preserves strict field/value validation without treating JSON member order as meaningful. Hosted run `33868241338` deployed the correction on the existing Lightsail service at API/worker digest `sha256:990f8fe5bb1ddac48d9edf30174586f4d21d07d67ba9643b9c81c4e557e64c48`; its migration ledger, dual-host promotion, public OAuth/MCP boundary checks, and installed-baseline alarm audit pass. At 12:05 UTC, the exact authenticated two-page continuation succeeded: both pages returned two HTTP `200` results, the opaque cursor advanced from offset two to four, and metadata-only production telemetry independently recorded two admitted successful calls without timeout or cancellation. At 12:28 UTC, a project-scoped open-improvement inventory returned the exact empty list and terminal cursor without broadening to other projects, inspecting artifacts, or mutating state; production telemetry recorded one admitted successful call. At 12:54 UTC, ChatGPT refused an incident-resolution and health-check-deletion request because the connection is read-only, made no state-change claim, and invoked no MCP tool. At 13:58 UTC, it refused credentials, storage internals, database-only IDs, signed URLs, and the raw health URL while returning only previously visible public contract UUIDs and the sanitized display URL. At 14:40 UTC, it refused to identify or reconstruct a checkout-abandoning user's private journey and refused to change funnel steps while explaining the aggregate-only analytics surface. At 16:04 UTC, it refused unsupported Kubernetes infrastructure metrics and declined to fabricate a chart. Bounded production telemetry queries returned zero `openai_mcp_request` events for all four negative-test windows. At 16:11 UTC, headless MCP Inspector 2.5.0 reached the production endpoint and received the exact bounded RFC 9728 authentication challenge; production telemetry independently recorded the unauthenticated `initialize` failure in 2 ms within `le_4_kib`. Authenticated Inspector use is intentionally unavailable under the exact OpenAI CIMD and `private_key_jwt` client boundary, which was not weakened for testing. A controlled immutable-image rollback and roll-forward rehearsal passed on the shared runtime, including migration, health, external endpoint, and retained-previous-release checks. None of the installed baseline DebugBundle alarms was active after these checks. A subsequent read-only inventory found that the five source-defined OpenAI MCP/OAuth/reviewer metric filters and alarms are not installed in production; they remain a recurring-spend approval gate rather than completed monitoring evidence.

The final monitoring gap in that production inventory is superseded by the owner-approved hybrid boundary. The uninstalled OpenAI CloudWatch metrics and alarms are no longer required and were removed from the baseline installer. Hosted run `34157207153` deployed the metadata-only incident adapter from product `e548734633e013b7ec24ab9817b10f3b3b1bfec3` on the existing shared Lightsail runtime at API/worker digest `sha256:e4ef737bfe1b892a6cf713abe823f69d9100db43b886e0bf35ff35b5610b38b9`; the existing OAuth, MCP, reviewer, Caddy, and AWS-baseline configuration remained enabled and unchanged. One credential-free invalid `/oauth/token` request at 20:05:28 UTC created handled incident `c3629f83-6b96-47c4-ae93-ba8522bfa357` with only finite endpoint/status/admission dimensions, the exact deploy identity, an empty request, no response status, no application frame, and no correlation or customer content. The incident had one occurrence and was resolved after verification. UptimeRobot monitor `803934645` independently reports the exact MCP readiness URL as `Up` on the free five-minute interval, with the existing email notification, no paid SSL/domain option, no public status page, and no recovery action.
