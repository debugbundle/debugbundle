...
## Analytics queue compatibility correction

Core 1.9.2 routes analytics bundle processing through the same typed `dequeue` contract as the durable worker lane. The processor does not claim or acknowledge Redis directly. The lane adopts ingress before processing and completes its journal receipt only after the processor returns. Generation failures retain their existing explicit failed state; queue or metadata-read failures use journal retry. A lost journal completion after artifact publication retries the receipt without rebuilding or failing the completed generation. This fixes the idle-loop failure and analytics processing regression in 1.9.0/1.9.1 without a schema or public interface change. Runtime composition and real Postgres/Redis/object-storage regressions cover idle polling, successful generation, read failure and completion failure.
