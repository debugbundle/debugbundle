...
**FR-SDK-19b:** In the next hardened SDK release, every automatic log adapter and public log capture method must reject records below the effective local level and server capture policy before event construction, message formatting, user hooks, privacy traversal, or SDK-owned retention. An ineligible record cannot be promoted by `beforeSend`. A separate, explicitly enabled and bounded breadcrumb policy may admit INFO context; it does not change the WARNING default. Preserve intentional standalone INFO capture when the effective level and policy admit it. Changing the documented legacy hook order requires an explicit major-version migration and compatibility tests.
...
**FR-SDK-35:** Wave 2 backend SDKs (Java, C#, Kotlin server, Rust) must implement the universal SDK interface with language-idiomatic naming conventions and at minimum one first-class framework integration each: Spring Boot (Java), ASP.NET Core (C#), Ktor (Kotlin server), Axum + Actix Web (Rust). Java exception runtime facts must use the canonical `{ rss, heap_total, heap_used, external, peak }` memory object, keep JVM-only facts under `framework_extras`, and pass the shared strict event envelope contract before release. See `spec/sdk-language-targets.md` for per-language guidance and `contracts/sdk-interface.md` for the full interface contract.
...
Monitor-internal errors are unverified executions, not evidence that the customer endpoint failed or recovered. They must retain raw diagnostic results without advancing either threshold, changing the last known customer health, opening or resolving a customer incident, or sending customer alerts/webhooks. The worker must report these errors to operators in bounded aggregates.
...
**FR-GRP-10:** Severity inference must preserve high severity for backend exceptions, non-opaque frontend exceptions, immediate request-failure incident signals, and a legacy Java redirected-stderr ERROR root exception header from a recognized stderr logger. Continuation lines from that stream are context signals, not separate incidents; similarly shaped messages from ordinary Java application loggers remain incident signals. Low-confidence opaque browser-native `frontend_exception` captures must not be stamped high by default: opaque `window_error` signals infer `low`, and opaque `resource_error` signals infer `medium`. `error_suppressed` events infer `medium`; other events infer `low` unless explicitly escalated by a later operator-controlled path.
...
**FR-BND-11:** Browser resource bundles may add optional lifecycle interruption evidence and a bounded list of failed recovery requests that followed a sampled resource occurrence by at most 30 seconds and shared its session or trace. Recovery candidates must have retained source evidence, sanitized relative request paths, return `4xx`/`5xx`, and have recovery semantics such as refresh, renew, retry, recover, or signed-URL/presign. Bundle diagnosis prioritizes a correlated recovery failure over generic resource-error guidance while stating that correlation does not establish the original failure cause. Bundle v1 and old artifacts remain compatible.
...
**FR-ALT-05:** Opaque browser resource failures from the same project, service, environment, browser session, page origin/path, resource host and interruption class share a separate privacy-safe burst key within fixed 10-second capture-time buckets, without changing incident fingerprints or existing per-resource cooldown identities. Email retains eligible distinct incidents in project/recipient digests with configurable 1–300-second windows (default 10). Immediate channels enforce both burst suppression and configured cooldowns atomically; project-scoped cooldowns are opt-in per alert rule, isolated by environment and severity. Signed lifecycle webhooks retain their event stream.

**FR-ALT-06:** Causally related rapid incidents must create one durable, bounded notification group per eligible project, alert rule, destination, service, environment, severity, and lifecycle transition, with one initial logical delivery for Slack, email, Discord, and alert webhooks. Preserve every accepted member incident ID, count and retrieval link; keep separate failures, critical escalations, later regressions, and recipient scopes independently eligible. Concurrent workers, retries, out-of-order events, and worker restarts must not multiply logical intents or lose accepted members. Provider timeouts remain at-least-once at the external boundary. Existing signed lifecycle webhooks keep their per-event payload and delivery semantics unless a recipient explicitly selects a versioned summary mode. Project-scoped group and delivery inspection must be available through API, CLI, and MCP without exposing raw log text or destination secrets.

**FR-ALT-07:** Custom alert-webhook rules must sign the exact transmitted JSON body with a per-rule HMAC-SHA256 secret returned only on opted-in creation or explicit rotation. Store the secret outside readable rule config and never expose it in list or ordinary update responses. Every custom direct-alert delivery is signed. Legacy rules receive an independent persisted key before delivery and retain their existing JSON body and management response shape; explicit rotation reveals a new key and upgrades the body to include group references. Missing signing material fails closed. Slack, Discord, alert-webhook, signed lifecycle-webhook and weekly Slack report sends must validate the public destination at socket connection time and reject redirects; hosted availability checks must revalidate each redirect and pin its connection address.
...
**FR-CLI-15:** Agent-aware setup supports verified `codex`, `claude-code`, `gemini-cli`, and `muse-code` integrations through repeatable `--agent` selection and an interactive detected-agent chooser. `--agent none` removes only proven-owned integrations. Non-interactive/JSON setup without selection retains legacy behavior and any saved selection. Repeated setup preserves reviewed profiles, connections, and user content. Native guidance links/copies derive from the one canonical generated `.agents/skills/debugbundle/` tree; no separate skill package, external installer, or implicit MCP configuration is introduced. Muse Code reuses its documented instruction precedence and canonical skill root; shared instruction ownership survives agent deselection and template upgrades.

**FR-CLI-16:** Doctor and validate report canonical guidance separately from selected native discovery, distinguishing missing, stale, conflicting and undiscoverable states. Ownership metadata and content hashes bound repair/removal to unchanged generated artifacts; user edits and unsafe symlinks remain untouched. Native instruction sections and gitignore updates preserve surrounding user bytes and existing canonical imports. Copy fallback is deterministic, bounded, and repairable, retaining unknown files and directories. Reads reject unsupported file types and invalid UTF-8; JSON failures remain structured for automation and MCP. Existing project roots are reused from nested directories within the repository boundary.
...
**FR-MCP-16:** All DebugBundle generated/project and plugin skills must use CLI-first capability routing where the host can execute the applicable command, while honoring explicit user interface choices and MCP-only hosts. They must inspect safe connection metadata, use separate saved CLI auth for hosted operations, establish exact project/current-record scope, retain explicit user authorization, and verify writes. A read-only connector must not become an overall capability limit. Missing shell/CLI/auth, denied access, unsupported commands, ambiguous scope, and uncertain writes must remain distinct. Shared generated guidance and cross-distribution parity tests must cover Codex, Claude Code, Gemini CLI, OpenAI, portable skills, and every future Muse or other plugin before release. No implicit installation, credential transfer, access-denial bypass, or expansion of hosted MCP tools/scopes is allowed. See `contracts/agent-interface-routing.md` and `spec/openai-plugin-cli-handoff.md`.

**FR-MCP-17:** Gemini CLI developer support must provide a native extension containing the shared DebugBundle workflow skill and an exact published `@debugbundle/mcp` stdio pin with `--local-auth`, plus a documented direct MCP alternative. The extension must start local tools in the active workspace, preserve project-skill precedence, use saved CLI/member authentication without model-visible credentials, and keep setup, installation, login, and writes separate. Its independently versioned root-manifest package must be staged as a standalone installable archive for Gemini's distribution rules. A dedicated guide must cover install, authentication, verification, update/removal, duplicate configuration precedence, and release status. Existing Codex, Claude, CLI, default MCP, and hosted OpenAI contracts remain compatible. See `spec/gemini-developer-integration.md` for package validation, native acceptance, and public release steps.
...
**FR-AUTH-04b:** Browser redirect callbacks controlled by external providers, including GitHub sign-in, GitHub App setup, and Slack OAuth, must validate every consumed or security-relevant query field and strip unknown query fields before domain logic runs. Malformed or duplicated consumed fields must fail closed. Signed provider webhooks are outside this compatibility rule and retain their signature and payload validation requirements.
...
**FR-ONB-04:** The `.agents/skills/debugbundle/SKILL.md` must follow the agentskills.io specification with YAML frontmatter and progressive disclosure. Must be discoverable by all major AI agent frameworks (Codex, GitHub Copilot, Cursor, Cline, Claude Code, custom MCP clients). No protocol coupling — skill files are plain Markdown with structured sections.
...
**FR-EVT-02:** Free tier billing must primarily meter Class A (incident signal) events. Class B and Class C events must not count against the primary `monthly_raw_ingested_events` allowance on Free. On paid tiers (Solo, Team), Class A and Class B events count against the allowance; Class C operational signals are excluded on every tier.

**FR-EVT-02a:** Hosted allowance enforcement must make every whole unit in the active tier-and-capacity limit usable. A valid operation that brings usage exactly to the limit is allowed; only work beyond the limit is rejected or suppressed. When a debug or analytics ingestion batch exceeds the remaining monthly allowance, the API must admit eligible events in input order through the exact remaining boundary, return indexed quota errors only for the surplus, and leave events excluded from that meter unaffected. Non-reservation counters must not reserve a conservative margin below the limit; concurrent requests may produce a small visible overage instead of an early rejection. A limit-reached notification must never report usage below its limit.
...
**FR-EVT-07b:** Browser capture-rule matchers may constrain `browser_event.page.visibility_state`, `browser_event.page.ready_state`, `browser_event.target.tag_name`, and the bounded target attributes `rel`, `as`, `type`, `media`, `cross_origin`, `async`, `defer`, and `integrity_present`. Requested fields use exact normalized matching and fail closed when evidence is absent. Until installed SDKs understand these predicates, rules that use them stay out of SDK config and are evaluated only by authoritative hosted ingestion so an older client cannot silently broaden a demote or drop decision.
...
**NFR-PERF-01:** SDK overhead must be minimal — no synchronous blocking, bounded memory for event queues, graceful drop under backpressure. Automatic capture and logger/framework hooks must not wait for transport, local file persistence, remote configuration refresh, another event's privacy/serialization work, or a full queue. Bounds include retained events, breadcrumbs, suppression state, retries, in-flight batches, scheduled work, bridges, and offline storage. Explicit flush may wait only within a documented finite deadline and must not hold a lock that delays concurrent capture. Optional Node/browser/React Native application callbacks execute after capture returns on their JavaScript event loop and must return promptly; their arbitrary closure execution cannot be isolated transparently. SDK-owned queue/network/overload limits remain mandatory. The owner-approved PHP/WordPress standard-install exception permits one small, best-effort HTTP attempt at request end, after capture has completed, with an advisory short network timeout and possible event loss; it may still occupy a PHP worker. No automatic capture method may initiate that attempt, and PHP/WordPress must not be described as strictly nonblocking or reliably asynchronous.
