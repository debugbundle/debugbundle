...
Current package versions, MCP pins, public channels, and release evidence for all three developer distributions are recorded in the [agent distribution ledger](agent-distribution-ledger.md).
...
The final standalone ZIP contains exactly the five reviewed source files and has SHA-256 `16719d47e15bdbe987cd74be42bcb21a3a461c1818c5f94880f8d3c3b258f025`. Public-source installation, deployment, and gallery discovery are tracked as separate release checks in the ledger.
...
2. Publish the exact standalone files at the root of the public extension repository, create a matching version tag, then publish a GitHub Release on that tag with the verified root-layout ZIP attached. A tag alone does not appear as a GitHub Release. Keep core as the maintained source; do not independently edit the published copy.
3. Run `make gemini-extension-verify-public` after publishing the standalone repository to confirm its five tracked files are byte-identical to core and its version tag resolves to public `main`. Verify a fresh `gemini extensions install <public-repository-url>`, the installed version/skill/MCP connection, local evidence, update/reinstall, and removal from that public source. Update candidate wording and installation instructions using the verified URL.
...
5. Add the `gemini-cli-extension` GitHub topic and a matching version tag when gallery publication is authorized. Verify the gallery entry independently after its daily crawler runs; a topic, tag, or successful installation alone does not establish indexing.
