...
| Alert group inspection                       | `GET /v1/alert-groups`, `GET /v1/alert-groups/{kind}/{id}`           | `alert groups/group`                      | `list_alert_groups/get_alert_group`                                                                                   | Browser Session or Member Token, scoped to project; safe metadata and paginated incident identities only                                                                                                                                    |
...
### 1.2b AnalyticsBundle And Product Analytics
...
**Analytics opportunity list response:** `{ "opportunities": AnalyticsOpportunityRecord[], "next_cursor": "string | null" }`.

**Analytics opportunity detail response:** `{ "opportunity": AnalyticsOpportunityRecord }`.
  ...
  "samples": [
    {
      ...
      "visitor_id_hash": "sha256:... | null",
      "analysis_tags": ["checkout"],
...
**Analytics journey sample response:** `GET /v1/analytics/journey-samples/{id}?project_id=<uuid>` returns:
...
Email `config` also accepts `aggregation_window_seconds` (integer 1–300, default 10). Eligible incidents to the same project/recipient join the first pending digest without extending its deadline. Slack, Discord and alert-webhook configs accept `cooldown_scope: "incident" | "project"` (default `incident`). Project scope applies the rule's cooldown across distinct incidents, separately per environment and severity, so escalation remains eligible. Distinct alert rules and signed lifecycle webhook subscriptions remain independent. Browser resource burst suppression uses a separate fixed ten-second capture-time bucket key; delayed worker processing cannot reopen the same bucket, and this does not replace per-resource cooldown identity. All custom alert-webhook deliveries are signed. To receive the secret at creation and opt into group-reference payloads, include `signing: "hmac_sha256_v1"` alongside `channel: "webhook"` in the create request. Updated CLI/MCP clients add this opt-in automatically and must be released after the API accepts it. The response returns the per-rule `alert.signing_secret` once, and the worker signs the exact transmitted JSON with HMAC-SHA256 in `X-DebugBundle-Signature: sha256=<hex>`. Store the secret in the receiver; later list and ordinary update responses omit it. Older strict clients omit `signing`, so their create response and JSON body remain compatible. New legacy-shaped rules receive a private key at creation; historical rules receive one atomically before delivery. Concurrent evaluators share the persisted winner key. Explicit rotation lets the owner configure receiver verification. Missing keys fail delivery closed. Rotate an existing rule using `PATCH /v1/alerts/{id}?project_id=...` with `{"channel":"webhook","rotate_signing_secret":true}`; the response reveals the new secret once. CLI uses `debugbundle alert update <id> --project-id <id> --channel webhook --rotate-signing-secret true`; MCP uses the additive `rotate_alert_webhook_secret` tool; the existing `update_alert` tool schema stays unchanged. Rotation changes the active key immediately, so update the receiver before expecting it to verify later deliveries. No extra host service or global signing-key setting is required.
Large email digests retain all accepted matches in storage while the email renders at most 25 severity-prioritized distinct incidents, preserves each sampled incident's matched condition labels, and reports the full distinct-incident count. It links to the project incident list when the application base URL is configured. This affects email presentation only; it does not alter alert rule matching or lifecycle webhook delivery.

Examples: `{"to":"ops@example.com","aggregation_window_seconds":30}` and `{"target_url":"https://example.com/alerts","cooldown_scope":"project"}`. CLI passes these through `--config-json`; ordinary MCP passes them through `config` in `create_alert`/`update_alert`. No new SDK version is required for hosted enforcement. Lifecycle matcher management through distributed clients requires a CLI/MCP release built with the updated shared schemas; old installed clients remain compatible with their existing fields. Until that release, use the API for lifecycle-aware matchers: older management clients can omit unknown predicates and must not create or replace those matchers.
...
`rotate_signing_secret: true` is accepted only with `channel: "webhook"`; it returns a one-time `alert.signing_secret` and preserves other rule fields. `signing_secret` is stored separately from readable alert config and never appears in list responses.
...
- Severity-threshold rules use `severity_lifecycle_scope` to match new incidents, incident regressions, or both. The default is `both`; delivery dedupe keys retain the source-transition identity for regressions so retries of one transition remain idempotent while a later regression of the same incident can notify independently. The stable notification key remains separate and continues to enforce any configured cross-transition cooldown.
...
- Delivery transport is implemented for `channel: "email"`, `channel: "slack"`, `channel: "discord"`, and `channel: "webhook"`. Email requires `config.to` as a single recipient address; Slack accepts either `config.slack_destination_id` (resolved to an encrypted stored webhook URL at delivery time) or `config.webhook_url`; Discord requires `config.webhook_url`; webhook requires `config.target_url`. Alert create/update returns `400 alert_target_blocked` for a private/reserved literal target, embedded credentials, a non-HTTP(S) scheme, or a port other than 80/443. The worker also rejects private/reserved DNS answers before connecting and does not follow redirects. Existing rules with now-blocked targets remain stored but fail delivery with a bounded `alert_target_blocked` or `alert_redirect_blocked` reason; update them to a direct public endpoint. All custom alert-webhook rules sign their payloads independently of the lifecycle-webhook subscription system. Legacy bodies retain their field set; explicit rotation reveals the new verification secret and enables group-reference fields.
- Authorization failure: `401 { "error": "invalid_member_token" }`
- Invalid list query: `400 { "error": "invalid_query" }`
...
- `POST /v1/projects/{id}/slack/destinations/{destinationId}/test` requires owner/admin access, decrypts the stored webhook URL, sends a Slack test message through the guarded public-target transport with a five-second deadline and no redirects, and returns `502` with a stable Slack-specific error code when delivery fails.
...
Lifecycle webhook delivery validates public HTTP(S) targets on ports 80/443 at connection time, rejects redirects and private/reserved destinations, and keeps the existing per-event signed payload identity. An existing webhook pointed at a blocked target remains configured but fails delivery until its URL is changed to a direct public endpoint.
...
Request body matches the shared capture-rule schema and supports `action: "demote" | "sample" | "drop"` plus matcher fields such as `event_types`, `services`, `environments`, `runtime`, `browser_event_kind`, `browser_event_opaque`, `browser_page_visibility_state`, `browser_page_ready_state`, `browser_target_tag_name`, `browser_target_attributes`, `client_kind`, `bot_family`, `message_equals`, `message_contains`, `error_name`, `resource_url`, `request_url`, `status_codes`, `first_party`, and optional `fingerprint`. `browser_target_attributes` accepts the bounded keys `rel`, `as`, `type`, `media`, `cross_origin`, `async`, `defer`, and `integrity_present`; string fields are trimmed and lowercased and every requested field must be present and equal. Exact-fingerprint matchers are evaluated at ingestion against the server-derived, versioned canonical fingerprint used by worker grouping; clients do not provide or control the evaluated fingerprint.

If any active rule uses page lifecycle, target tag, or target-attribute predicates, `GET /v1/sdk/config` returns an empty capture-rule list. Hosted ingestion evaluates the entire set. This preserves rule precedence when older SDKs cannot evaluate every predicate, including broad local drops overridden by narrower hosted rules.
...
Agent-aware setup and repair follow [the agent setup contract](../spec/agent-aware-cli-setup.md): repeatable `--agent` selection, canonical `.agents/skills/debugbundle/`, selected native instruction blocks, Claude relative-link/bounded-copy discovery, and versioned ownership metadata. `doctor`/`validate` add `agent_setup` for declared selections; missing/stale/conflict/undiscoverable states are separate from profile validation. Repairs preserve user edits, profiles, connections, and unrelated skills. Legacy non-interactive runs do not infer vendors. `--agent none` removes only unchanged owned native integrations. No plugin or MCP installation is implicit.

Current local CLI setup behavior: `debugbundle setup [--agent <codex|claude-code|gemini-cli|muse-code|none>]... [--non-interactive] [--json]` is the single entrypoint that replaces the old `debugbundle init`. It detects mixed-runtime service roots, creates missing `profile.json` with `validation_status: "static-analysis-only"`, preserves an existing cloud connection or creates `.debugbundle/local/connection.json` (defaults to `"mode": "local-only"`), generates `.agents/skills/debugbundle/SKILL.md` per agentskills.io spec, manages `.gitignore` entries, and optionally appends a lightweight DebugBundle trigger section to `AGENTS.md`. The generated skill uses capability-first trigger language for runtime error/crash reporting, incident reporting/response, live-app and production monitoring, endpoint health checks, debug bundles, and product analytics, while explicitly limiting monitoring/observability claims to runtime failures, customer-facing incidents, and endpoint health. It teaches a runtime-evidence-gated investigation workflow: check incidents first for qualifying runtime, production, health-check, notification, webhook, or captured-artifact issues, but inspect source/tests first for deterministic local UI, layout, copy, calculation, refactor, or test-only issues unless runtime evidence is needed. It also covers profile validation, browser relay verification, incident resolution hygiene, and noise-management review using capture-rule suggestions or path-scoped client-error capture policy when repeated low-value incidents are supported by bundle evidence. In interactive TTY sessions it lets the operator choose which detected targets to prepare first; in non-interactive mode it keeps deterministic whole-repo defaults. When the repository already includes browser and backend SDK surfaces, setup either scaffolds deterministic relay insertion points (Fastify, Express, Next.js App Router) or emits runtime-specific relay guidance for shipped relay-capable runtimes. JSON output exposes `detected_services`, `selected_targets`, `relay_action`, and `relay_guidance` so agents can act on the same machine-readable state as the human-readable output.
...
debugbundle analytics summary --project-id <id> [--from <ISO8601>] [--to <ISO8601>] [--last <duration>] [--json]
...
debugbundle alert groups --project-id <id> [--limit <n>] [--cursor <cursor>] [--auth-file <path>] [--json]
debugbundle alert group <direct|email_digest> <group-id> --project-id <id> [--limit <n>] [--cursor <cursor>] [--auth-file <path>] [--json]
...
Claude Code distribution: `.claude-plugin/marketplace.json` registers `debugbundle@debugbundle` from `apps/mcp/claude-code/debugbundle`. Plugin 1.10.0 opts into the same local-auth profile through its pinned MCP 1.10.0 command. Its optional `member_token` and `api_url` settings still map to server environment values; blank settings fall back to CLI auth. Updating this plugin does not alter the default stdio schema or other agent packages. Installation, update and removal are covered by `/docs/mcp/claude-code/`.
...
debugbundle_get_usage_summary → same result as GET /v1/analytics/summary
...
debugbundle_list_analytics_journey_samples → same result as GET /v1/analytics/journey-samples
debugbundle_get_analytics_journey_sample → same result as GET /v1/analytics/journey-samples/{id}
...
debugbundle_list_alert_groups   → same result as `GET /v1/alert-groups`
debugbundle_get_alert_group     → same result as `GET /v1/alert-groups/{kind}/{id}`
...
For ChatGPT connection discovery, `/mcp` accepts a zero-length `application/octet-stream` POST and routes it through the same canonical-host, rate-limit, and OAuth-challenge path as an empty unauthenticated JSON request. This compatibility parser is scoped only to `/mcp`. Any non-empty binary body, and `application/octet-stream` on any other API route, remains unsupported with `415 Unsupported Media Type`; binary data never reaches the MCP transport or an authenticated domain reader.
...
Each schema advertised through `tools/list` is independently resolvable: every local `#/$defs/...` reference is accompanied by that tool schema's transitive definitions. The wire projection normalizes array-form nullable `type` declarations to equivalent `anyOf` branches for MCP client portability. This is serialization-only; the normative fixture semantics, strict runtime input validation, and successful output validation do not change.
...
The hosted API dogfoods DebugBundle for alert-worthy MCP request failures, MCP timeouts, MCP admission rejections, OAuth failures, and reviewer-credential expiry. Request-derived incident dimensions contain only finite endpoint/method/tool/status/admission/expiry metadata. Before delivery, the adapter empties request/response placeholders, clears correlation, removes probe data and executable stack frames, and retains only Node version from runtime context. The normal event ID/time, service/environment/runtime, SDK name/version, and internal ingestion-authentication envelope remain. Incidents exclude request IDs, pseudonymous grant/client keys, OAuth credentials, interaction identifiers, tool arguments/results, exception detail, customer IDs, and customer content. Normal unauthenticated MCP discovery and rejected bearer tokens remain metadata logs rather than incidents. A monitoring adapter is fail-safe and runs outside the OAuth/MCP domain transaction, so reporting cannot change a customer-visible result.

Independent reachability and TLS monitoring checks only `https://mcp.debugbundle.com/ready` from UptimeRobot at the free-tier five-minute interval and alerts the account's existing email contact. Monitor `803934645` was independently observed `Up` after creation on 2026-09-07. It never triggers shared-host recovery or bypasses the MCP-only Caddy gate. Existing AWS baseline alarms remain unchanged. The baseline AWS monitoring installer must not create the five superseded OpenAI-specific CloudWatch custom metrics or alarms.
...
The fixtures are the normative contract for the implemented source and deployed Developer Mode candidate. `tests/fixtures/openai-plugin-v1/implementation-gaps.json` records the successful owner reconnect, captured non-secret Developer Mode connection mapping, the complete owner-authenticated 23-tool production corpus across fresh Codex clients, retained ChatGPT boundary cases, and the owner-approved deployed keyboard/screen-reader check separately from the still-unavailable synthetic reviewer corpus, reviewer-isolation, submission, publication, and directory evidence. MCP Inspector 2.5.0 independently reaches the production Streamable HTTP endpoint in headless mode and receives the exact bounded RFC 9728 authentication challenge; production telemetry records the unauthenticated `initialize` rejection. Authenticated Inspector use is intentionally unavailable because the production authorization server admits only OpenAI's exact CIMD client with `private_key_jwt`; validation must not extract a ChatGPT bearer token or weaken that client boundary. The equivalent authenticated `initialize` and `tools/list` path already passes through ChatGPT. A data-free local Inspector harness scans the exact 23-tool catalog with strict mode and reports no schema errors or portability warnings. Hosted run `33907119277` deployed the self-contained `$defs` and portable nullable-union projection on the same Lightsail runtime at API/worker digest `sha256:349c98954955643f3f14cb1070623bbf7c25d3e6b1402d35214bca9479d3fdb7`; all 48 migrations were already applied, the existing OpenAI/Caddy configuration was preserved, and independent public API/MCP/OAuth checks pass. On 2026-09-08 fresh authenticated Codex clients enumerated exactly the frozen 23-tool catalog and exercised every tool with no more than two concurrent calls. All calls completed without mutation; empty analytics remained explicit, health URLs stayed sanitized, and an existing failed improvement artifact remained an honest bounded result. The endpoint-health evidence also preserves the resolved production pagination defect: the owner passed the exact server-issued opaque cursor unchanged with the same project, check, lookback, and limit, initially reproducing a failure before its hosted reader ran. A red regression reproduced the order-sensitive strict-input comparison when the client appended `cursor` after `limit`; strict validation now compares parsed JSON structure without treating object member order as significant. Source tests and hosted deployment run `33868241338` pass. The owner then repeated the authenticated flow: pages one and two each returned two HTTP `200` health results, and the opaque cursor advanced from offset two to four. Metadata-only production telemetry independently recorded both calls as admitted successes without timeout or cancellation. A later project-scoped `list_improvements` request returned the exact empty list and terminal cursor; the client did not broaden scope, inspect an artifact, or mutate state. The retained negative mutation, secret-exfiltration, individual-analytics-journey, and generic-infrastructure cases also pass. ChatGPT refuses unavailable state changes without claiming mutation; refuses credentials, storage internals, database-only IDs, signed URLs, and raw health URLs while distinguishing allowed public contract UUIDs and sanitized display URLs; refuses identities/private journeys plus funnel mutation while explaining the aggregate-only alternative; and declines unsupported Kubernetes telemetry rather than fabricating it. All four answers made no MCP call, as independently confirmed by their bounded production telemetry windows. Hosted run `34157207153` deployed the owner-approved hybrid monitoring source on the shared Lightsail API process at API/worker digest `sha256:e4ef737bfe1b892a6cf713abe823f69d9100db43b886e0bf35ff35b5610b38b9` while retaining the existing AWS baseline. One credential-free invalid `/oauth/token` request created exactly one handled incident whose bounded context retained only endpoint/status/admission and deployment metadata plus an empty request envelope; it retained no body, credential, client, redirect, identity, header, correlation value, or customer content, and was resolved after verification. UptimeRobot monitor `803934645` independently reports the MCP readiness URL `Up`.
