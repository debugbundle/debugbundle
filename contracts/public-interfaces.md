...
V1 CLI auth uses a member token created in the web app or API. `debugbundle login` persists `{ bearer_token, base_url }` to `~/.debugbundle/auth.json`, and `debugbundle whoami` reads the same local auth state.

Current local CLI behavior: read-path commands and token-management commands reuse that stored auth state when constructing authenticated API requests, so local users and agents do not need to inject bearer tokens manually once `debugbundle login` has been completed.
