...
The fixtures are the normative contract for the implemented source and deployed Developer Mode candidate. `tests/fixtures/openai-plugin-v1/implementation-gaps.json` records the successful owner reconnect, captured non-secret Developer Mode connection mapping, two passing ChatGPT cases, one passing fresh-thread Codex aggregate-analytics case, and one passing primary endpoint-health sequence separately from the still-unavailable complete Inspector/ChatGPT/Codex corpus, accessibility, reviewer, submission, publication, and directory evidence. The endpoint-health evidence also preserves a confirmed production pagination defect: the owner passed the exact server-issued opaque cursor unchanged with the same project, check, lookback, and limit, and the second page failed before its hosted reader ran. A red regression reproduced the order-sensitive strict-input comparison when the client appended `cursor` after `limit`; strict validation now compares parsed JSON structure without treating object member order as significant. Source tests and hosted deployment run `33868241338` pass; the authenticated live second-page recheck remains open.
