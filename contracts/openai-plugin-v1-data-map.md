...
## Internal Operational Monitoring Boundary
...
The incident adapter replaces the ambient Fastify request and response with empty placeholders and replaces executable stack frames with the finite signal message. Its delivery sanitizer also clears correlation and removes probe data plus process platform, architecture, PID, working directory, uptime, hostname, and memory. The remaining standard event envelope is limited to event ID/time, service name/environment/runtime, SDK name/version, Node version, and the project token required by the existing internal ingestion transport. That token authenticates DebugBundle to its own ingestion API and is never an OpenAI input, output, log field, incident field, or external-monitor value. These operational envelope fields never include the triggering request, response, tool input/output, OAuth credential, user, organization, customer project, grant, client, or customer content.
