...
| `log_event`           | `incident_signal` or `context_signal`    | `incident_signal` when `level` is `error`, `fatal`, or `critical`, except Java redirected-stderr stack continuation lines (`Caused by:`, `at`, `... N more`, and related frames) from `stderr` or `org.jboss.stdio` logger identities, which are `context_signal` and attach to the parent exception when the SDK can assemble it; `context_signal` otherwise. An ordinary Java application ERROR with similar text remains an incident. |
...
An SDK may also emit a bounded aggregate for local queue pressure. It retains the required fields above and adds `reason: "queue_pressure"` plus a bounded `level` class such as `info`, `warning`, `error`, or `exception`. `suppressed_count` is the number omitted in that class. Consumers must treat these as operational counts, not separate application incidents.
...
| signing_secret     | text        | Internal per-rule HMAC key; nullable for non-webhook rules and legacy rows awaiting provisioning |
| webhook_payload_version | smallint | NOT NULL DEFAULT 0, CHECK IN (0, 1) |
| is_enabled         | boolean     | NOT NULL DEFAULT true  |
| created_at         | timestamptz | NOT NULL DEFAULT now() |
| updated_at         | timestamptz | NOT NULL DEFAULT now() |

Migration `202609250002_version_alert_webhook_payloads` preserves legacy custom webhook JSON with version 0 and marks existing explicitly signed rules as version 1. Every custom alert webhook is signed; eligible legacy rules atomically acquire a stable per-rule key before delivery. Explicit key creation or rotation selects version 1 with group references. Provider-native Slack/Discord requests retain their existing formats. Keys are excluded from ordinary management and group-read responses; explicit creation/rotation reveals the key once.
...
Opaque browser resource incidents retain per-resource notification identities and use a separate optional `coalescing_key` in `alert_deliveries`. This hashed key includes project/service/environment/session, page origin/path, resource host, interruption class and a fixed ten-second capture-time bucket. Alert evaluation scopes the burst key by severity so escalation is not suppressed by a lower-severity delivery. Immediate channels must pass both the existing configured cooldown and the burst check; the burst key remains effective through worker backlog rather than expiring with delivery wall-clock time. Distinct lifecycle transitions and later capture-time buckets remain eligible when configured cooldown permits. Database transactions lock before the suppression read snapshot. Email retains distinct per-incident items in the project/recipient digest. Email `config.aggregation_window_seconds` accepts 1–300 seconds, default 10. Immediate-channel `config.cooldown_scope` accepts `incident` (default) or `project`; project scope remains isolated to the selected rule, environment and severity. No raw session identifier or page query is stored in a notification key.

`browser_recovery_events` is an internal reference index introduced by migration `202609220001_add_browser_recovery_context`: `event_id` UUID primary key, cascading `project_id`, `service_name`, `environment`, `kind` (`resource` or `recovery`), `occurred_at`, nullable project-scoped `session_hash`/`trace_hash` (at least one required), and `expires_at`. Expiry is at most seven days after the earlier of capture and insertion. No request payload, URL or raw correlation value is stored. Scheduled retention prunes bounded batches; project deletion cascades. Indexes support scoped time-window reads and expiry. Requests and standalone network breadcrumbs can be indexed even when classified as context.
...
The worker loads a severity-prioritized sample of at most 25 digest matches plus the exact distinct-incident count for rendering. The email reports omitted incidents and links to the project's incident list. All accepted digest item rows remain stored; this render bound does not cap pending database membership.
A claimed digest whose scheduler stopped before durable enqueue becomes claimable again after ten minutes. Once enqueued, its durable delivery job retries a failed provider attempt with bounded backoff against the same digest ID; a timed-out provider may have accepted the first attempt, so email delivery is at-least-once. Failure diagnostics store a bounded internal code rather than provider exception text.
...
### Browser resource incident evidence

`bundle_version` remains `1`. Optional `context.resource_failure` uses `version: 1` and contains sanitized `host`, exact resource `path`, element `type`, nullable `first_party`, `role`, nullable `provider`, `title`, `optional_candidate`, `diagnosis`, and `routes`. Roles are `analytics`, `advertising`, `tag_manager`, `authentication`, `application_asset`, or `unknown`. Origin and dependency role are independent. Absolute targets without a page origin stay unknown; root-relative targets are same-origin even when the host is unavailable; `optional_candidate` is a suggestion eligibility hint, never an automatic severity or capture decision.

The same block may add `interruption` with exact `visibility_state` (`hidden`, `prerender`, or `unloaded`), `ready_state` (`loading` or `interactive`), `target_tag_name: "link"`, and `rel` (`preload`, `modulepreload`, or `prefetch`). It may also add up to ten `recovery_failures`, each with `source`, uppercase `method`, sanitized query-free `path`, `status_code`, ISO `occurred_at`, and `delay_ms` capped at 30 seconds. Recovery evidence is limited to correlated retained request events or frontend network breadcrumbs with explicit recovery-path semantics. Both additions are optional so old Bundle v1 artifacts remain valid.

`routes` contains at most 20 `{ route, occurrences }` entries, sorted by descending count then route, plus `recorded_occurrences`, `unattributed_occurrences`, `omitted_routes`, and `coverage` (`occurrence_metadata` or `retained_samples`). Counts are a bundle-generation snapshot. Samples must not be presented as a complete impact count. Routes omit query/fragment and normalize dynamic path segments; resource paths preserve asset filenames and versions. Bundle readers must accept old artifacts without this optional context. The fixed OpenAI v1 projection remains unchanged; its existing summary fields carry the cautious diagnosis.

Fingerprint v3 applies only to opaque `resource_error` events with a concrete HTTP(S) host, path and recognized DOM element type. It hashes environment, resource kind, host, exact path, type and algorithm revision; page route, generic error text and SDK stack frames are excluded. Project/service/environment storage isolation remains in force. Missing identity retains prior grouping. All unrelated event hashes remain v2-compatible. Installed v1/v2 exact-fingerprint rules use bounded, server-derived legacy aliases; neither rule records nor historical incident hashes are rewritten.

Forward migration `202609160001_add_browser_resource_routes` adds nullable `incident_events.resource_route`. Worker jobs carry optional `incident_title` and `resource_route`; old queued jobs remain accepted. The sanitized route survives raw-event sampling and uses the existing incident/event idempotency key. Old rows remain null; no backfill or historical merge runs. Required-migration readiness and the current-bootstrap sentinel include this migration. Run `db:migrate` before new API/worker code starts; `db:bootstrap` remains empty-schema-only. Rollback code can ignore the added column.

### Alert delivery membership candidate

Forward migration `202609250001_add_alert_evaluation_owner` adds nullable `alert_deliveries.evaluation_job_id`, recording the existing durable evaluation job ID when a new direct intent is created. Only that job may retry its pending or failed send; later occurrence jobs remain deduplicated even if their incident keys and capture timestamps are identical. The owner ID is internal and never appears in provider payloads or inspection output. Historical rows remain null and are not assigned an invented retry owner. The nullable column has no journal foreign key, so normal job-receipt retention remains independent. Run migrations before new API/worker code starts; required-migration readiness and clean-schema baseline checks include this change, and older runtimes may ignore the column.

Forward migration `202609240001_add_alert_delivery_members` adds `alert_delivery_members` with `id`, `delivery_id`, `incident_id`, `condition_type`, `dedupe_key`, and `created_at`. `(delivery_id, incident_id, condition_type, dedupe_key)` is unique; `delivery_id` cascades when its delivery is removed. The incident ID is retained without a foreign key so a member's identity survives individual incident retention until the owning delivery expires. The `(delivery_id, created_at, id)` index supports stable pagination. New direct alert intents atomically record their root incident, and later events sharing an eligible coalescing key attach their incident IDs without creating another direct delivery, including when the provider marked the first attempt failed. The same durable root evaluation job can retry its pending or failed delivery after a crash or provider error; later members do not trigger independent retries, and a retry does not consume a second delivery allowance. A provider may have accepted an attempt that timed out, so external delivery remains at-least-once rather than exactly-once. One direct delivery retains at most 1,000 membership rows; a longer same-key burst starts a new delivery rather than silently discarding accepted member identities. Member replay checks earlier deliveries with the same coalescing key even after their membership cap is reached, so a replay cannot attach the same incident to a later spill group or create a second logical alert; a later independent capture-time bucket stays eligible. Both direct delivery and email digest writes verify that the alert rule and incident belong to the same project before creating the delivery/digest. Existing historical deliveries are not backfilled by this migration; the inspection reader includes their root `incident_id` as a synthetic legacy member in counts and paginated member pages until it is backfilled. If a later member joins such a delivery, its root is recorded atomically with that member when both fit within the cap. A full historical delivery is never expanded past 1,000 rows by root backfill on replay; its root identity remains on the delivery record and in inspection even if its member row cannot be added. Historical direct deliveries whose root incident or alert rule belongs to a different project are excluded from group inspection. Existing direct member incident IDs from another project are excluded from counts and pages; a retained member ID remains visible when its incident has already been deleted. Historical email digest items whose incident or alert rule belongs to another project are excluded from group inspection and delivery rendering. Member pages recheck project and organization scope after the group lookup. The migration is additive, required before API/worker code starts using membership, and safe for older runtime code to ignore. This is a source candidate, not the complete bounded, four-channel alert-group contract in FR-ALT-06.
