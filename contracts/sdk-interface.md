...
On `pagehide` and hidden `visibilitychange`, direct mode must use authenticated `fetch(keepalive)` because `sendBeacon()` cannot set the required `Authorization` header. Relay mode may use credential-free `sendBeacon()` and falls back to credential-free keepalive fetch. Lifecycle requests from one SDK instance must share a budget below the browser's 64-KiB outstanding-body limit across debug and analytics lanes. Browser 3.0.2 reserves 60 KiB in total; settled keepalive requests release capacity, while accepted beacons keep their reservation because no completion signal is available. Other page code may consume the same browser quota. A declined or failed lifecycle request retains unsent events for ordinary delivery while the page remains active; acceptance by `sendBeacon()` establishes only that the browser queued the request, not server delivery. Ordinary and lifecycle fetches apply the same bounded rate-limit backoff and authorization rejection rules. Explicit flush waits for already-owned lifecycle sends within its existing finite deadline. Acknowledged records remain charged until every overlapping sender releases them, and a late response must not requeue an already acknowledged record. Built-in direct HTTP transport requires a canonical acknowledgement; explicitly supplied custom transports and legacy relays retain bodyless-success compatibility.
