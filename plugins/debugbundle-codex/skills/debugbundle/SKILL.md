...
2. Follow CLI-first capability routing. Use CLI `doctor` or the appropriate project-scoped incident/health read. When MCP is selected, confirm its server in `/mcp` and its working directory for local evidence.
