...
Plugin 1.1.1 keeps the local CLI as the primary interface for supported DebugBundle operations, with explicit user interface choices and MCP-only hosts preserved. Its MCP server pin is the published 1.12.1. Authorization, exact project scope, and readback apply to every write.
...
With MCP 1.12.1 published to npm and this package on the repository's default branch:
...
The marketplace is `.agents/plugins/marketplace.json`; its package is `plugins/debugbundle-codex`. This repository marketplace is a distribution source, not an OpenAI directory approval. The plugin pins `@debugbundle/mcp@1.12.1` with `--local-auth` and includes no credentials, custom transport, background hooks, or hosted app mapping.
...
```bash
codex mcp add debugbundle -- npx -y @debugbundle/mcp@1.12.1 --local-auth
```
