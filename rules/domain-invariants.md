...
**Enforcement:** Integration tests must verify that standalone `request_event` payloads outside the preset's immediate failure set are rejected on a Free project with `minimal` preset, first-party 5xx `request_event` payloads are accepted under every preset/override, `balanced` projects accept 429 request failures by default, `investigative` projects accept 409 request failures even when `capture_request_events` is otherwise narrowed, and explicit client-error path rules promote only matching `4xx` status+path+method requests.
