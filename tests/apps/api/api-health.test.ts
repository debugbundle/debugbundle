...
function createServer(options: Parameters<typeof createApiServer>[1] = {}): ReturnType<typeof createApiServer> {
...
describe("api health routes", () => {
  ...
  it("should expose security.txt for the public API surface", async (): Promise<void> => {
    ...
    expect(response.body).toContain("Contact: https://github.com/debugbundle/debugbundle/security/advisories/new");
    expect(response.body).toContain("Policy: https://github.com/debugbundle/debugbundle/security/policy");
    expect(response.body).toContain("Canonical: https://api.debugbundle.com/.well-known/security.txt");
  });
});
