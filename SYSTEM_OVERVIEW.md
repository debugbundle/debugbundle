...
Browser resource incidents use a resource-only fingerprint v3 (exact host/path/element type across page routes); other events remain v2. Shared evidence interpretation supplies provider/asset titles and cautious possible-cause wording. Capture rules can now match page visibility/readiness, target tag, and bounded link attributes at authoritative ingestion; a project with lifecycle-aware rules keeps its full rule set at ingestion to preserve mixed-version SDK precedence. Hidden incomplete speculative link failures infer low severity and can suggest confirmation-required first-party demotion. Separate asset incidents can share a ten-second capture-time burst key while retaining distinct fingerprints, per-resource cooldowns and email digest items. Optional Bundle v1 `context.resource_failure` carries bounded route counts plus lifecycle interruption and correlated recovery-failure evidence; a following recovery `4xx`/`5xx` is highlighted without claiming causation. Migration `202609220001_add_browser_recovery_context` adds expiring correlation references and optional alert burst keys after the earlier route-retention migration; historical incidents remain unchanged. See `spec/browser-resource-incidents.md` for validation and release boundaries.
...
Agent-aware CLI setup keeps one generated project skill in `.agents/skills/debugbundle/`. `packages/agent-setup` owns safe native instruction/discovery integration, append-only gitignore updates, and hash-based repair; CLI templates remain in `local-scaffold.ts`. Selected Codex/Claude Code/Gemini CLI/Muse Code integrations, ownership metadata, additive doctor/validate reports, and rollback are specified in `spec/agent-aware-cli-setup.md`. Non-interactive legacy behavior and existing project/cloud configuration are preserved. Installed-package gates cover the Node runtime matrix, upgrades from the published predecessor, and offline native Muse skill validation/discovery. Shared instruction ownership preserves blocks needed by another selected agent.

The public MCP docs provide dedicated setup guides at `/docs/mcp/codex/` and `/docs/mcp/claude-code/`, with plugin/direct-connection choices and version-specific authentication guidance. The Claude Code plugin 1.11.0 uses MCP 1.11.0's `--local-auth` profile while preserving its existing member-token/API settings. Default stdio schemas and the hosted OpenAI connection are unchanged.
...
The resource origin is permanent; OAuth issuer is exactly `https://api.debugbundle.com`, with frozen JWKS path `/oauth/jwks.json`. `get_incident_context` requires incident and artifact scopes and excludes raw logs. Health URLs are sanitized before projection. The same Lightsail API image/process is protected by a Redis-coordinated, database-aware MCP bulkhead, atomic dual-host Caddy promotion, and an independently operable MCP-only no-deploy Caddy gate. The runtime, forward migration, projection, owner-approved existing-app consent/reviewer/revocation UI, opt-in development-only synthetic UI preview, package, private-cloud deployment source, and automated local evidence are implemented. Production migration, managed Caddy, DNS/TLS, OAuth/reviewer configuration, and MCP activation are deployed for owner-approved Developer Mode validation; the observed empty binary ChatGPT discovery probe has a bounded compatibility parser and live `401` challenge evidence. The owner reconnect, Developer Mode connection mapping, three bounded ChatGPT cases, a fresh-thread Codex aggregate-analytics case, and an owner-client endpoint-health case are captured. The health case proved the primary sanitized read-only sequence and then exposed an order-sensitive strict-input comparison on second-page requests. The owner confirmed the exact server cursor and unchanged query fields; a red public-handler regression reproduced the defect, and source now validates JSON objects independent of member order. Hosted run `33868241338` deployed that correction on the shared Lightsail API/worker service with all 48 migrations already applied; external API/MCP/OAuth boundary checks pass, and none of the currently installed baseline DebugBundle alarms is active. The owner then completed the exact authenticated two-page continuation: both calls returned two HTTP `200` results, advanced the opaque cursor from offset two to four, and were independently corroborated by successful metadata-only production telemetry. A project-scoped open-improvement request also returned the exact empty result without broadening scope, inspecting artifacts, or mutating state. Subsequent negative mutation, secret-exfiltration, individual-analytics-journey, and generic-infrastructure requests were refused without any MCP call or claimed state change. The secret case exposed only public contract identifiers and the sanitized health display URL, not credentials, storage internals, database-only IDs, signed URLs, or raw URLs. The analytics case refused identity/private-journey access and funnel mutation while explaining the aggregate-only surface, and the infrastructure case refused unsupported Kubernetes telemetry rather than fabricating it; bounded production telemetry independently recorded no request in all four negative-test windows. The controlled immutable-image rollback and roll-forward rehearsal passes on the shared runtime. Engineering privacy/security reconciliation, reconciled policy deployment, automated keyboard interaction checks, and the owner-approved deployed keyboard/screen-reader spot check pass; remaining owner legal attestations stay separate gates. The five uninstalled OpenAI CloudWatch filters/alarms are superseded by the owner-approved hybrid monitoring boundary and are not a launch gate. The rotated reviewer credential and outside-network fixed-tenant isolation smoke pass. The remaining retained synthetic reviewer corpus, representative load evidence, portal review, publication, and directory evidence remain separate gates.

The 2026-09-07 monitoring decision supersedes the uninstalled five-alarm plan described above. Alert-worthy MCP request failures/timeouts/admission rejections, OAuth failures, and reviewer expiry now flow through fail-safe metadata-only handled DebugBundle incidents; normal MCP discovery and rejected bearer tokens remain logs. Independent `https://mcp.debugbundle.com/ready` monitoring runs in the existing free UptimeRobot account, while the installed AWS baseline remains unchanged and its installer is prevented from creating the superseded OpenAI custom metrics. Monitor `803934645` reports `Up` on the free five-minute interval. Hosted run `34157207153` deployed the incident adapter on the same Lightsail API process at immutable API/worker digest `sha256:e4ef737bfe1b892a6cf713abe823f69d9100db43b886e0bf35ff35b5610b38b9`; one credential-free invalid-token request created exactly one sanitized handled incident and retained no request body, credentials, identity, headers, correlation, or customer content. The intentional incident was resolved after verification.

On 2026-09-08 the owner supplied and approved Owen Far as the Malta-based operator/controller, the concise controller/processor and international-transfer policy boundary, and every country supported by the OpenAI portal as intended availability. This supersedes the earlier pending-owner-legal wording above. The corresponding updated privacy/terms source still requires a separate site release and rendered verification; OpenAI publisher-identity verification remains a manual portal gate.

The deployed release candidate materializes self-contained advertised tool schemas and normalizes nullable array-form `type` declarations into portable `anyOf` branches. A pinned, data-free MCP Inspector 2.5.0 strict scan reports no schema errors or warnings across the exact 23-tool catalog. Hosted run `33907119277` promoted the correction on the same Lightsail runtime at API/worker digest `sha256:349c98954955643f3f14cb1070623bbf7c25d3e6b1402d35214bca9479d3fdb7`, preserved the existing OAuth, reviewer, MCP, and managed-Caddy configuration, and found all 48 migrations already applied. Independent public API/MCP/OAuth boundary checks pass. This wire-format correction preserves the frozen schema semantics. Post-deploy authenticated catalog refresh remains separate live-client evidence; full operator evidence lives in the private hosted runbook.

On 2026-09-08, fresh authenticated Codex clients loaded the corrected three-starter-prompt package, enumerated exactly the frozen 23-tool catalog, and exercised every tool against production with at most two concurrent calls. Every call completed without mutation; empty analytics remained explicit, health URLs remained sanitized, and a failed existing improvement artifact remained a bounded result. This supersedes the earlier partial owner-client corpus and pending catalog-refresh wording above. The outside-network reviewer isolation smoke also passes after the owner-authorized credential rotation. The synthetic reviewer retained corpus and representative load evidence remain separate gates. Hosted-MCP API processes now record their event-loop p95 by atomically overwriting one metadata-only file inside the container, allowing the bounded capacity canary to close the event-loop gate without adding an endpoint, customer record, external metric, or recurring monitoring stream.
...
The bounded worker scheduler also performs a leased scheduled analytics opportunity pass every six hours by default. It selects only enabled projects with recent daily aggregate activity in bounded UUID-cursor batches, reuses the existing deterministic evaluator, and never scans raw analytics objects; event-triggered evaluation remains the low-latency path.

Worker idle waits retain only currently active polls and cancel timers on shutdown; they must not accumulate reactions on a process-lifetime shutdown promise. Frequency snapshot throttling is capped at 10,000 incident IDs, temporary readiness S3 clients are destroyed, and unused worker HTTP responses are cancelled. `make worker-memory-check` verifies retained idle-poll memory independently of traffic; production RSS stability still requires a post-deploy observation window. See `spec/worker-resource-lifecycle.md`.
...
| Queue           | Redis lists + processing claims                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| Public Site     | Static-exported Next.js + Fumadocs on `debugbundle.com` for marketing, MDX-backed `/docs` and `/blog`, `llms.txt`, `openapi.json`, schemas, and example artifacts. Marketing content authored: real landing page (hero, how-it-works, value props, interfaces overview, terminal example, OpenGraph), pricing page (Free/Solo/Team with tier features and shared-allowance model from `spec/product.md` section 13), 4 blog posts, and substantive legal/informational pages (about, contact, privacy, terms, security, changelog). Documentation content authored per `spec/documentation.md` (65-page surface with Orama static search: getting started, SDKs, CLI, API, MCP, webhooks, probes, bundles, security, self-hosting, agent workflows). |
...
packages/
  storage/       — Storage adapters and domain persistence
    src/
      ...
      availability-check-store.ts — Postgres availability-check CRUD, execution claiming/recording, incident linking, and 30-day result/rollup retention; shared queries allocate organization execution slots only after excluding per-project excess
      ...
      redis-queue.ts         — Redis queue client (enqueue/claim/ack; atomic stale recovery in batches of 100)
...
See `/spec/product.md` section 13 for the finalized pricing source of truth. Three tiers: **Free → Solo → Team**.
...
| Hosted health checks | 1/project, 3 monitored projects, 3 active, 5m minimum | 3/project, 10 monitored projects, 30 active, 1m minimum | 10/project, 10 monitored projects, 50 active, 1m minimum |
| Shared dashboards    | —                                                     | —                                                       | ✅                                                       |
...
### Incident reliability boundaries
...
The worker adopts Redis ingress into the Postgres `worker_jobs` journal before acknowledgement. Normalization and grouping commit domain metadata with follow-up intents on one connection; nested domain transactions use savepoints. Incident stages receive fair processing passes, outbound delivery runs in an independent bounded lane, and optional improvement failures retry independently. Lifecycle publication depends on a successful required bundle build. The existing database pool, Redis client and worker process are reused.

`worker-job-store.ts` owns fenced claims, bounded retries/retention and metadata inspection; `durable-queue.ts` owns claim/transaction orchestration; `durable-incident-processing.ts` composes scoped domain stores. Processor, improvement and metadata implementations are split by responsibility with compatible barrels. Forward migration and paused candidate activation are required before rollout. Keep one active incident worker until concurrent artifact publication and full-path scale-out are proven. See `spec/worker-durability.md`.
...
### Browser resource noise controls (core 1.12.0 candidate)

Lifecycle/target capture predicates run at ingestion; a project with any such predicate keeps its full rule set server-side to preserve older SDK precedence. Resource fingerprints remain exact. A separate hashed capture-time burst key suppresses immediate alert fanout while original per-resource cooldowns continue working. Email retains every eligible incident and has a configurable 1–300-second project/recipient digest; project cooldowns are opt-in per immediate-channel rule and isolated by severity/environment. Lifecycle webhooks stay unchanged.
