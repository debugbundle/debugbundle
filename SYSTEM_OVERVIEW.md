...
4. **Normalize & Classify** — Worker parses raw events into canonical form and assigns `event_class` (`incident_signal`, `context_signal`, `operational_signal`). Classification is immutable after normalization. Exceptions, `error`/`fatal`/`critical` logs, and request events with `response_status >= 500` are incident signals; non-5xx request events remain context. Only `incident_signal` events create incidents. Free-tier billing counts only `incident_signal` events.
...
packages/
  ...
  sdk-browser/   — [MOVED to debugbundle-js repo] Browser SDK (global errors, standalone/error-only breadcrumbs, first-party 5xx request_event promotion, probes, session controls, config fetch, network filters, trace headers, unload-safe flush, retained-buffer retry/backoff)
...
Event billing uses the event class model: Free meters only `incident_signal` events (Class A — exceptions, qualifying logs, and 5xx request failures), paid tiers meter `incident_signal` + `context_signal` (Class A + B), and `operational_signal` (Class C) is excluded from billing across all tiers. Each project has a capture policy (preset: `minimal`/`balanced`/`investigative`) that controls what the SDK captures and the ingestion API accepts. Free defaults to `minimal` (failure-first, including 5xx request failures); paid defaults to `balanced`.
