...
3a. **Analytics Ingest (planned)** — Opt-in browser analytics events use the same SDK/relay/direct transport and `/v1/events` request envelope for transport simplicity, then split into a separate analytics lane. Analytics events are checked for project enablement, consent/quota policy, and privacy-safe schema boundaries, persisted only as short-lived raw analytics objects, and enqueued for aggregate processing. They do not receive debug `event_class` values and never create, reopen, regress, alert, webhook, or dispatch incident automation.
