...
3. **Ingest** — (Connected mode) API validates, enforces capture policy, evaluates capture rules (`capture_rule_dropped` / `capture_rule_sampled_out` when applicable), redacts, persists accepted raw events to object storage, and enqueues processing with the resolved capture preset plus resolved status-wide and path-scoped client-error incident rules so worker-side request classification stays deterministic. Demotion is enforced again in processing as a backstop. No synchronous incident/grouping persistence in request path. (Local mode) `debugbundle process` reads events from `.debugbundle/local/events/` and composes the same pure-function packages (event-normalizer, bundle-engine, repro-engine) to produce bundles locally.
