...
10a. **Retrieve Analytics (partial runtime)** — Humans and agents can fetch aggregate project metrics through `GET /v1/analytics/summary`, `/routes`, `/devices`, `/referrers`, and `/funnels/{key}`, matching `debugbundle analytics summary|routes|devices|referrers|funnel` commands, and MCP `get_usage_summary`, `get_route_metrics`, `get_device_breakdown`, `get_referrer_metrics`, and `get_funnel_analysis`. These responses stay aggregate-only and read from rollups. Remaining planned retrieval surfaces cover journey patterns, analytics opportunities, and AnalyticsBundle artifacts through API, CLI, MCP, and web. The main web sidebar gets an Analytics surface like Incidents/Improvements for cross-project opportunities and bundles; each project gets one Analytics tab with internal sections/sub-tabs for summary, routes, funnels, devices, referrers, opportunities, and generated bundles.
