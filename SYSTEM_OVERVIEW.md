...
10a. **Retrieve Analytics (partial runtime)** — Humans and agents can fetch project usage summary metrics through `GET /v1/analytics/summary`, `debugbundle analytics summary`, and MCP `get_usage_summary`; the response is aggregate-only and includes totals plus bounded device/browser/OS/language/referrer/auth-state breakdowns from rollups. Remaining planned retrieval surfaces cover route metrics, dedicated device/referrer/funnel views, journey patterns, analytics opportunities, and AnalyticsBundle artifacts through API, CLI, MCP, and web. The main web sidebar gets an Analytics surface like Incidents/Improvements for cross-project opportunities and bundles; each project gets one Analytics tab with internal sections/sub-tabs for summary, routes, funnels, devices, referrers, opportunities, and generated bundles.
