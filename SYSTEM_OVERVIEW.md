...
3a. **Analytics Ingest And Aggregation (partial runtime, foundation present)** — Shared analytics event, AnalyticsBundle, and metrics response schemas now exist, production migrations/bootstrap define project analytics settings, the analytics ingestion ledger, the unique-rollup ledger, aggregate rollups, journey sample indexes, analytics opportunities, and AnalyticsBundle generation metadata, and the project analytics settings control plane is wired through API, CLI, and MCP for owner/admin mutation plus member preview. `/v1/events` now splits mixed debug/analytics batches after project-token auth: debug events keep the existing capture-policy/classification/incident path, while enabled `analytics_event` inputs are validated against analytics schemas/settings/custom-dimension allowlists, persisted as short-lived `analytics-events/...` raw objects, and enqueued to the analytics aggregation queue. The worker now processes `aggregate-analytics-events` jobs by validating the gzipped raw analytics object, inserting the ingestion ledger row first, and updating hourly/daily session, route, route-transition, action/conversion/marker, and funnel rollups only when the event is new. Rollups are precomputed aggregate rows keyed by time bucket and bounded dimensions, not raw event logs or long-term per-visit timelines. Initial analytics opportunity evaluation runs in the same background aggregation transaction for relevant rollups: aggregate `funnel_dropoff` candidates above minimum sample/dropoff thresholds and bidirectional `journey_friction` route loops above minimum transition/session thresholds upsert stable `analytics_opportunities` rows with aggregate-only evidence. Direct metrics retrieval reads these aggregate rollups through API, CLI, and MCP for summary, routes, journey transition patterns, device/browser/OS/language breakdowns, referrer/UTM breakdowns, and funnel conversion/dropoff analysis. The standalone browser SDK now has opt-in analytics capture for session start, page view, route change with previous-route transition context, semantic action, funnel step, and conversion events through the existing browser transport, with analytics disabled by default and consent gating isolated from debug capture. Browser analytics and debug breadcrumbs should reuse privacy-safe SDK helpers for route/session/device/action context where possible, but they emit separate envelopes and debug capture continues when analytics is disabled, unavailable, sampled out, quota-blocked, or failing. The shared retention cleanup job now removes expired raw analytics objects, expired retained journey-sample objects/metadata, and aggregate rollups older than each project's configured aggregate retention window while keeping in-window aggregate metrics as the normal query model. Storage now includes the internal AnalyticsBundle generation metadata lifecycle for idempotent reservation, pending-job claiming, and completed/failed state propagation back to linked analytics opportunities; the Redis queue has an internal `build-analytics-bundle` job contract for later worker execution; and `packages/analytics-bundle-engine` now provides deterministic `AnalyticsBundleV1` shaping for selected aggregate/sample inputs. Analytics-specific allowance checks, broader scheduled opportunity evaluators, retained journey sample replay, and worker-side AnalyticsBundle input selection, artifact persistence, and retrieval are still planned. Analytics events do not receive debug `event_class` values and never create, reopen, regress, alert, webhook, or dispatch incident automation.
...
10a. **Retrieve Analytics (partial runtime)** — Humans and agents can fetch aggregate project metrics through `GET /v1/analytics/summary`, `/routes`, `/journey-patterns`, `/devices`, `/referrers`, and `/funnels/{key}`, matching `debugbundle analytics summary|routes|journeys|devices|referrers|funnel` commands, and MCP `get_usage_summary`, `get_route_metrics`, `get_journey_patterns`, `get_device_breakdown`, `get_referrer_metrics`, and `get_funnel_analysis`. These metric responses stay aggregate-only and read from rollups. Stored analytics opportunities are now readable through `GET /v1/analytics/opportunities` and `/opportunities/{id}`, matching `debugbundle analytics opportunities` / `analytics opportunity get` and MCP `list_analytics_opportunities` / `get_analytics_opportunity`. Internal storage can reserve and track AnalyticsBundle generation metadata, but remaining planned retrieval surfaces still cover public AnalyticsBundle artifact create/get plus retained journey sample details through API, CLI, MCP, and web. The main web sidebar gets an Analytics surface like Incidents/Improvements for cross-project opportunities and bundles; each project gets one Analytics tab with internal sections/sub-tabs for summary, routes, funnels, devices, referrers, opportunities, and generated bundles.
