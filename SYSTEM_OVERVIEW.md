...
The local release candidate now materializes self-contained advertised tool schemas and normalizes nullable array-form `type` declarations into portable `anyOf` branches. A pinned, data-free MCP Inspector 2.5.0 strict scan reports no schema errors or warnings across the exact 23-tool catalog. This wire-format correction preserves the frozen schema semantics and remains deployment-pending; hosted run `33868241338` is still the production authority.
