...
export function createRedisQueueClient(input: CreateRedisQueueClientInput): RedisQueueClient {
  ...
  async function enqueue(
    jobName: "normalize-events" | "group-incident" | "build-bundle" | "build-reproduction" | "evaluate-alerts" | "deliver-webhook" | "deliver-github-dispatch" | "generate-weekly-report" | "cleanup-retention",
    payload: NormalizeEventsJob | GroupIncidentJob | BuildBundleJob | BuildReproductionJob | EvaluateAlertsJob | DeliverWebhookJob | DeliverGitHubDispatchJob | GenerateWeeklyReportJob | CleanupRetentionJob
  ): Promise<void> {
  ...
  async function dequeue(
    jobName: "normalize-events" | "group-incident" | "build-bundle" | "build-reproduction" | "evaluate-alerts" | "deliver-webhook" | "deliver-github-dispatch" | "generate-weekly-report" | "cleanup-retention"
  ): Promise<NormalizeEventsJob | GroupIncidentJob | BuildBundleJob | BuildReproductionJob | EvaluateAlertsJob | DeliverWebhookJob | DeliverGitHubDispatchJob | GenerateWeeklyReportJob | CleanupRetentionJob | null> {
    const raw = await redis.lpop(`jobs:${jobName}`);
    if (raw === null) {
      return null;
    }

    return JSON.parse(raw) as NormalizeEventsJob | GroupIncidentJob | BuildBundleJob | BuildReproductionJob | EvaluateAlertsJob | DeliverWebhookJob | DeliverGitHubDispatchJob | GenerateWeeklyReportJob | CleanupRetentionJob;
  }
