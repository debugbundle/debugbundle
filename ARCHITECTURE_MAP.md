...
- **Internal structure:**
  ...
  - `routes/ingestion.ts` — event ingestion; planned AnalyticsBundle support keeps `analytics_event` transport on `/v1/events` but splits accepted analytics inputs into a separate analytics lane instead of incident grouping
  - `routes/incidents.ts` — incident retrieval, bundle, reproduction, logs
  - planned `routes/analytics.ts` — AnalyticsBundle aggregate metrics, opportunities, and artifact retrieval/generation routes over shared analytics domain services
...
- **Planned analytics lane:** AnalyticsBundle processing owns aggregate rollups, journey sample retention, analytics opportunity evaluation, AnalyticsBundle generation, and analytics retention cleanup. It must be idempotent, aggregate-first, and separate from incident grouping so analytics events never create or update incidents.
...
- **Internal structure:**
  ...
  - planned analytics storage modules — project analytics settings, analytics ingestion ledgers, aggregate rollups, journey sample indexes, analytics opportunities, and AnalyticsBundle generation metadata
...
| ---------- | ----------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| PostgreSQL | `apps/api`, `apps/worker`                             | 44 relational tables today (users, organizations, organization_members, projects, project_tokens, member_tokens, services, incidents, incident_events, processed_events, improvement_opportunities, improvement_opportunity_events, bundle_generations, alert_rules, alert_deliveries, alert_email_digests, alert_email_digest_items, agent_webhooks, webhook_deliveries, weekly_report_channels, weekly_report_deliveries, probe_activations, deployments, org_usage_counters, project_usage_counters, capture_policies, github_installations, project_github_repos, github_dispatch_rules, github_dispatch_deliveries, audit_logs, password_credentials, sessions, email_verification_tokens, password_reset_tokens, invites, oauth_identities, processed_billing_events, plan_cleanup_tasks, account_analytics_accounts, account_metric_periods, account_metric_events, account_payment_retention_records, account_payment_provider_events); planned AnalyticsBundle tables add settings, ingestion ledger, rollups, journey sample indexes, opportunities, and bundle-generation metadata |
