...
- **Internal structure:**
  ...
  - `analytics-metrics-store.ts` — Postgres aggregate AnalyticsBundle summary, route, journey-pattern, device, referrer, action, and funnel metric queries over session/route/transition/action/funnel rollups for API/CLI/MCP retrieval, including bounded retained journey sample IDs on transition patterns
