...
The current local SDK repo set includes `debugbundle-js`, `debugbundle-python`, `debugbundle-php`, `debugbundle-wordpress`, `debugbundle-java`, `debugbundle-go`, `debugbundle-ruby`, and the active `debugbundle-android` local repo.
...
Detailed implementation plans live in `spec/sdks/java-sdk.md`, `spec/sdks/go-sdk.md`, `spec/sdks/ruby-sdk.md`, `spec/sdks/csharp-sdk.md`, `spec/sdks/kotlin-sdk.md`, and `spec/sdks/swift-sdk.md`; Java, Ruby, and Go now follow their plans in local standalone repos, the Kotlin Android repo now has its core/runtime, offline queue, crash/ANR replay, native HTTP instrumentation, UI/logging adapters, and probe-directive slice implemented locally against its plan, C#/.NET is planned as the next Wave 2 SDK repository, and Swift iOS remains at the plan stage for Wave 3.
