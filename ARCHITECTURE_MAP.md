...
In the JavaScript companion, Node `delivery-hooks.ts` finalizes reserved batches; Browser `event-transport.ts` owns bounded deferred preparation, transport and ACK reconciliation. React Native's synchronous preparation helper returns only the protected final canonical/legacy wire representation before awaiting the native bridge. Application hooks run after capture returns, on the JavaScript event loop, with explicit callback responsibility. The major-3 migration guides document changed timing. Root `privacy-js-candidate-check` qualifies both SDK archives against exact independently versioned core archives; WordPress `check-staged-release.py` consumes those archives and the PHP ZIP, and `smoke-browser-asset.mjs` executes the extracted IIFE with actual PHP-generated configuration before installed WordPress smoke.
