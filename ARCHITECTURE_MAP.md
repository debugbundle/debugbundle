...
- **Internal structure:**
  ...
  - `routes/analytics.ts` — AnalyticsBundle aggregate metrics read routes; currently implements usage summary over rollups, while route/device/referrer/funnel/journey/opportunity/artifact routes remain planned
...
- **Analytics lane:** `aggregate-analytics-events` validates short-lived raw analytics objects and calls the Postgres rollup store. It inserts the analytics ingestion ledger before updating hourly/daily session, route, action/conversion/marker, and funnel rollups, with `analytics_rollup_uniques` preserving exact unique-session counters. Usage summary retrieval now reads these aggregates through API/CLI/MCP. Journey sample retention, transitions, analytics opportunity evaluation, AnalyticsBundle generation, remaining detailed metrics, and analytics retention cleanup remain planned. The lane stays separate from incident grouping so analytics events never create or update incidents.
...
- **Analytics CLI ownership:** `management-analytics-command-handlers.ts` now routes `debugbundle analytics summary` and `debugbundle analytics settings get|set`; `analytics-metrics-commands.ts` owns the usage-summary API client and CLI rendering, while `analytics-settings-commands.ts` owns settings.
...
- **Analytics MCP ownership:** `analytics-metrics-tools.ts` exposes `get_usage_summary` over the shared CLI HTTP client, and `analytics-metrics-tool-catalog.ts` owns its published tool metadata. Settings remain in `analytics-settings-tools.ts` / `analytics-settings-tool-catalog.ts`.
...
- **Internal structure:**
  ...
  - `analytics-metrics-store.ts` — Postgres aggregate AnalyticsBundle usage-summary queries over session/action rollups for API/CLI/MCP retrieval
