...
- **OpenAI distribution ownership:** `apps/mcp/openai/debugbundle/` owns the independently versioned combined skill/package, while `apps/mcp/openai/submission/` and `release-manifest.json` own non-secret listing/test/release artifacts. `.app.json` remains absent until a real Developer Mode connection ID is captured through the supported package workflow; the owner-visible ChatGPT version ID is not substituted for it. The package intentionally has no `.mcp.json`. `tests/fixtures/openai-plugin-v1/implementation-gaps.json` preserves the live/manual evidence boundary.
