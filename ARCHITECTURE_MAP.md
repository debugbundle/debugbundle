...
- **Owns:** Implemented first seven web management slices plus hosted improvement management, project product-analytics overview/routes/funnels/audiences/journeys/opportunities, customer workspace Analytics inventory, and internal operator analytics: `/login`, `/signup`, `/verify-email`, `/forgot-password`, `/reset-password`, `/dashboard`, `/analytics`, `/analytics/workspace`, `/analytics/workspace/bundles`, `/settings`, `/member-tokens`, `/projects`, `/improvements`, `/projects/:projectId/improvements`, `/projects/:projectId/improvements/:improvementId`, `/projects/:projectId/analytics`, `/projects/:projectId/analytics/routes`, `/projects/:projectId/analytics/funnels`, `/projects/:projectId/analytics/audiences`, `/projects/:projectId/analytics/journeys`, `/projects/:projectId/analytics/journeys/:sampleId`, `/projects/:projectId/analytics/opportunities`, `/projects/:projectId/analytics/opportunities/:opportunityId`, `/projects/:projectId/settings`, `/projects/:projectId/members`, `/projects/:projectId/tokens`, `/projects/:projectId/probes`, `/projects/:projectId/webhooks`, `/projects/:projectId/alerts`, `/projects/:projectId/github`, `/invite`, and `/billing`
...
- **Internal structure:** `src/app.tsx` owns protected shell composition while `src/app-project-routes.tsx` owns the single project route tree reused by browser and memory routers, keeping route definitions behaviorally identical and below the source-file size limit. `src/pages/workspace-analytics-page.tsx` owns the `/analytics/workspace` opportunity and bundle inventory tabs while preserving exact `/analytics` for the internal operator dashboard. `src/pages/project-analytics-layout.tsx` owns project Analytics settings/tier gating, horizontally scrollable Overview/Routes/Funnels/Audiences/Journeys/Opportunities navigation, and shared bounded filters. Metric pages own aggregate reads; retained journey detail uses a defensive allowlist normalizer. `src/pages/project-analytics-opportunities-page.tsx` owns the project-scoped cursor inventory, and `src/pages/project-analytics-opportunity-detail-page.tsx` renders only known bounded aggregate evidence fields, deterministic evaluator thresholds, related incidents/deploy references, and bundle state rather than dumping arbitrary evidence JSON. `src/components/system/analytics-opportunities-table.tsx` is shared by workspace and project inventories so detail links and status formatting remain aligned. Bundle generation/detail remain planned. General management pages remain under `src/pages`; internal operator analytics stays isolated at exact `/analytics`. `src/lib/api.ts` remains the compatibility barrel over focused browser adapters. `src/components/ui/*` holds shadcn primitives and `src/components/system/*` holds reusable product components.
