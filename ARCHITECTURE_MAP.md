...
- **Internal structure:** `src/app.tsx` owns route composition and protected-page gating for the auth, admin analytics, customer workspace analytics, projects, improvements, workspace health-status, project settings/tools, organization members, billing, incidents, and project product-analytics surfaces. `src/pages/workspace-analytics-page.tsx` owns the `/analytics/workspace` opportunity and bundle inventory tabs while preserving exact `/analytics` for the internal operator dashboard; reusable workspace filters and inventory tables live under `src/components/system/workspace-analytics-*`. `src/pages/project-analytics-layout.tsx` owns project Analytics settings/tier gating, internal Overview/Routes/Funnels/Audiences/Journeys navigation, and the shared bounded time/service/environment query context. The corresponding project pages own aggregate summary, route, funnel, audience, and journey-pattern reads; `src/pages/project-analytics-journey-sample-page.tsx` owns retained journey detail through a defensive allowlist normalizer that renders only normalized routes, bounded signal/deploy context, fixed safe dimensions, and derived analysis tags. It must never render raw artifact objects, hashes, trace/event IDs, arbitrary custom dimensions, target text, DOM, screenshots, or video. Opportunity detail and bundle generation/detail remain planned. General management pages remain under `src/pages`; `src/pages/admin-analytics-page.tsx` owns the internal aggregate-only `/analytics` dashboard and mirrors API `404` denial semantics in the SPA. `src/lib/api.ts` is the compatibility barrel for cookie-backed browser requests, with focused Analytics, artifact/lifecycle, and additive-field normalization adapters in sibling modules. Browser capture-rule and Slack API adapters remain separate. `src/lib/session.tsx` owns session bootstrap, `src/lib/theme.tsx` owns theme synchronization, `src/components/ui/*` holds shadcn-based primitives, and `src/components/system/*` holds reusable product components.
