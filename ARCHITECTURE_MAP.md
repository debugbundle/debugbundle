...
- **Owns:** Implemented first seven web management slices plus hosted improvement management, project product-analytics overview/routes/funnels/audiences/journeys/opportunities/bundles, customer workspace Analytics inventory, and internal operator analytics: `/login`, `/signup`, `/verify-email`, `/forgot-password`, `/reset-password`, `/dashboard`, `/analytics`, `/analytics/workspace`, `/analytics/workspace/bundles`, `/settings`, `/member-tokens`, `/projects`, `/improvements`, `/projects/:projectId/improvements`, `/projects/:projectId/improvements/:improvementId`, `/projects/:projectId/analytics`, `/projects/:projectId/analytics/routes`, `/projects/:projectId/analytics/funnels`, `/projects/:projectId/analytics/audiences`, `/projects/:projectId/analytics/journeys`, `/projects/:projectId/analytics/journeys/:sampleId`, `/projects/:projectId/analytics/opportunities`, `/projects/:projectId/analytics/opportunities/:opportunityId`, `/projects/:projectId/analytics/bundles`, `/projects/:projectId/analytics/bundles/new`, `/projects/:projectId/analytics/bundles/:generationId`, `/projects/:projectId/settings`, `/projects/:projectId/members`, `/projects/:projectId/tokens`, `/projects/:projectId/probes`, `/projects/:projectId/webhooks`, `/projects/:projectId/alerts`, `/projects/:projectId/github`, `/invite`, and `/billing`
...
- **Internal structure:** `src/app.tsx` owns protected shell composition while `src/app-project-routes.tsx` owns the single project route tree reused by browser and memory routers, keeping route definitions behaviorally identical and below the source-file size limit. `src/lib/dogfooding.ts` owns the production AnalyticsBundle opt-in and fail-closed app route templating bridge: automatic pathname capture stays disabled, known UUID detail routes become stable parameter templates, and unknown identifier-shaped paths are dropped. `src/pages/workspace-analytics-page.tsx` owns the `/analytics/workspace` opportunity and bundle inventory tabs while preserving exact `/analytics` for the internal operator dashboard. `src/pages/project-analytics-layout.tsx` owns project Analytics settings/tier gating, horizontally scrollable Overview/Routes/Funnels/Audiences/Journeys/Opportunities/Bundles navigation, and shared bounded filters. Metric pages own aggregate reads; retained journey detail uses a defensive allowlist normalizer. `src/pages/project-analytics-opportunities-page.tsx` and `src/pages/project-analytics-bundles-page.tsx` own project-scoped cursor inventories. `src/pages/project-analytics-bundle-create-page.tsx` owns the full-page bounded generation form and canonical pending/ready/failed detail redirect. Opportunity detail renders only known bounded aggregate evidence fields, deterministic evaluator thresholds, related references, bundle state, and a completed-bundle link. AnalyticsBundle detail projects only allowlisted metrics, segments, patterns, representative journey events, references, recommendations, and redaction metadata; it never renders arbitrary artifact keys, IDs, custom dimensions, or fingerprints. `src/components/system/incident-analytics-impact.tsx` owns the additive incident-detail impact read, aggregate evidence projection, retained-journey links, and bounded incident-impact AnalyticsBundle action; failures there remain isolated from incident and debug-artifact retrieval. `src/components/system/analytics-opportunities-table.tsx` and `src/components/system/analytics-bundles-table.tsx` are shared by workspace and project inventories so detail links and status formatting remain aligned. General management pages remain under `src/pages`; internal operator analytics stays isolated at exact `/analytics`. `src/lib/api.ts` remains the compatibility barrel over focused browser adapters. `src/components/ui/*` holds shadcn primitives and `src/components/system/*` holds reusable product components.
