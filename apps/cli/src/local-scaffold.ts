...
export function buildCliReference(): string {
  return [
    ...
    "- `debugbundle alert groups --project-id <id> [--limit <n>] [--cursor <cursor>] [--auth-file <path>] [--json]` lists safe alert delivery and email digest groups; `debugbundle alert group <direct|email_digest> <group-id> --project-id <id> [--cursor <cursor>] --json` pages their incident IDs.",
...
export function buildMcpReference(): string {
  return [
    ...
    "## Notification Tools",
    "",
    "- `list_alerts`, `create_alert`, `update_alert`, `delete_alert` — manage incident alert rules. `list_alert_groups` and `get_alert_group` inspect project-scoped delivery groups without raw payloads or destination secrets.",
