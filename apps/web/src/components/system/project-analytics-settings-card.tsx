...
function buildFallbackDraft(input: ProjectAnalyticsSettingsCardProps): AnalyticsSettingsDraft {
  const capabilities = getTierCapabilities(input.organizationPlan);

  return {
    access_mode: input.canEdit ? "manage" : "preview",
    analytics_available: capabilities.analytics_bundle,
    ...defaultSettings,
...
function settingsPayload(draft: AnalyticsSettingsDraft): ProjectAnalyticsSettings {
  return {
    ...
    hourly_retention_days: draft.hourly_retention_days,
...
export function ProjectAnalyticsSettingsCard(
  props: ProjectAnalyticsSettingsCardProps
): JSX.Element {
  ...
  const canManage =
    hasLoadedSettings &&
    canEdit &&
    settingsDraft.access_mode === "manage" &&
    settingsDraft.analytics_available;
  const analyticsCapabilities = getTierCapabilities(organizationPlan);
  const customDimensionKeys = parseCustomDimensionKeys(
    settingsDraft.approved_custom_dimensions_text
  );
  const customDimensionsInvalid =
    settingsDraft.max_custom_dimensions > analyticsCapabilities.max_analytics_custom_dimensions ||
  ...
  async function handleSave(event: React.FormEvent<HTMLFormElement>): Promise<void> {
    ...
    setIsSaving(true);
    try {
      const response = await updateProjectAnalyticsSettings(
        projectId,
  ...
  return (
    <CollapsibleCard
      ...
      description="Configure opt-in browser usage capture, privacy, and bounded retention."
    ...
    >
      ...
      {!hasLoadedSettings || settingsDraft.analytics_available ? null : (
        <Notice title="Product analytics availability could not be resolved">
          Analytics settings are temporarily unavailable for this project. Try again after the
          project plan and capabilities have refreshed.
        </Notice>
      )}
      ...
      {canManage ? (
        <form className="flex flex-col gap-6" onSubmit={(event) => void handleSave(event)}>
          <FieldGroup>
            ...
            <Field>
              <FieldLabel id="project-analytics-privacy-label" htmlFor="project-analytics-privacy">
                Privacy mode
              </FieldLabel>
              <FieldDescription>
                Choose the identity and storage posture applied to browser analytics.
              </FieldDescription>
              <Select
                value={settingsDraft.privacy_mode}
                disabled={isLoading || isSaving}
                onValueChange={(privacyMode) =>
                  updateDraft({ privacy_mode: privacyMode as AnalyticsPrivacyMode })
                }
              >
                <SelectTrigger
                  id="project-analytics-privacy"
                  aria-labelledby="project-analytics-privacy-label project-analytics-privacy"
                  className="w-full"
                >
                  <SelectValue />
                </SelectTrigger>
              ...
              <FieldDescription>
                {
                  privacyOptions.find((option) => option.value === settingsDraft.privacy_mode)
                    ?.description
                }
              </FieldDescription>
            </Field>
            ...
            <SwitchField
              ...
              onCheckedChange={(consentRequired) =>
                updateDraft({ consent_required: consentRequired })
              }
            />
          </FieldGroup>
          ...
          <FieldSet>
            ...
            <FieldDescription>
              Choose the bounded product signals the browser SDK may emit.
            </FieldDescription>
            <FieldGroup className="grid gap-5 md:grid-cols-2">
              <SwitchField
                id="project-analytics-page-views"
                label="Capture page views"
                description="Record initial browser page views."
                checked={settingsDraft.capture_page_views}
                disabled={isLoading || isSaving}
                onCheckedChange={(capturePageViews) =>
                  updateDraft({ capture_page_views: capturePageViews })
                }
              />
              <SwitchField
                id="project-analytics-route-changes"
                label="Capture route changes"
                description="Record safe SPA navigation transitions."
                checked={settingsDraft.capture_route_changes}
                disabled={isLoading || isSaving}
                onCheckedChange={(captureRouteChanges) =>
                  updateDraft({ capture_route_changes: captureRouteChanges })
                }
              />
              <SwitchField
                id="project-analytics-actions"
                label="Capture semantic actions"
                description="Record explicitly named product actions and conversions."
                checked={settingsDraft.capture_actions}
                disabled={isLoading || isSaving}
                onCheckedChange={(captureActions) =>
                  updateDraft({ capture_actions: captureActions })
                }
              />
              <SwitchField
                id="project-analytics-friction"
                label="Capture friction signals"
                description="Record fixed repeated-click, dead-click, and backtrack markers."
                checked={settingsDraft.capture_friction_signals}
                disabled={isLoading || isSaving}
                onCheckedChange={(captureFriction) =>
                  updateDraft({ capture_friction_signals: captureFriction })
                }
              />
            </FieldGroup>
          </FieldSet>
          ...
          <FieldSet>
            ...
            <FieldDescription>
              Raw and journey evidence expires; hourly detail is bounded separately from long-lived
              daily aggregates.
            </FieldDescription>
            <FieldGroup className="grid gap-5 sm:grid-cols-2 xl:grid-cols-5">
              <NumberField
                id="project-analytics-sample-rate"
                label="Journey sample rate (%)"
                value={settingsDraft.journey_sample_rate * 100}
                min={0}
                max={100}
                step={1}
                disabled={isLoading || isSaving}
                onChange={(value) => updateDraft({ journey_sample_rate: value / 100 })}
              />
              <NumberField
                id="project-analytics-raw-retention"
                label="Raw retention (days)"
                value={settingsDraft.raw_retention_days}
                min={1}
                max={30}
                disabled={isLoading || isSaving}
                onChange={(rawRetentionDays) =>
                  updateDraft({ raw_retention_days: rawRetentionDays })
                }
              />
              <NumberField
                id="project-analytics-sample-retention"
                label="Journey retention (days)"
                value={settingsDraft.sample_retention_days}
                min={1}
                max={365}
                disabled={isLoading || isSaving}
                onChange={(sampleRetentionDays) =>
                  updateDraft({ sample_retention_days: sampleRetentionDays })
                }
              />
              <NumberField
                id="project-analytics-hourly-retention"
                label="Hourly retention (days)"
                value={settingsDraft.hourly_retention_days}
                min={1}
                max={analyticsCapabilities.analytics_hourly_retention_days}
                disabled={isLoading || isSaving}
                onChange={(hourlyRetentionDays) =>
                  updateDraft({ hourly_retention_days: hourlyRetentionDays })
                }
              />
              <NumberField
                id="project-analytics-aggregate-retention"
                label="Aggregate retention (months)"
                value={settingsDraft.aggregate_retention_months}
                min={1}
                max={120}
                disabled={isLoading || isSaving}
                onChange={(aggregateRetentionMonths) =>
                  updateDraft({ aggregate_retention_months: aggregateRetentionMonths })
                }
              />
            </FieldGroup>
          </FieldSet>
          ...
          <FieldSet>
            ...
            <FieldDescription>
              Allow only low-cardinality, non-sensitive keys approved for this project.
            </FieldDescription>
            <FieldGroup className="grid gap-5 md:grid-cols-[12rem_1fr]">
              <NumberField
                id="project-analytics-custom-dimension-limit"
                label="Custom dimension limit"
                value={settingsDraft.max_custom_dimensions}
                min={0}
                max={analyticsCapabilities.max_analytics_custom_dimensions}
                disabled={isLoading || isSaving}
                onChange={(maxCustomDimensions) =>
                  updateDraft({ max_custom_dimensions: maxCustomDimensions })
                }
              />
              <Field data-invalid={customDimensionsInvalid || undefined}>
                <FieldLabel htmlFor="project-analytics-custom-dimensions">
                ...
                />
                <FieldDescription>
                  Comma-separated keys using letters, numbers, dots, underscores, or hyphens.
                </FieldDescription>
                {customDimensionsInvalid ? (
                  <FieldError>
                    Use valid unique keys and keep their count within the selected limit.
                  </FieldError>
                ) : null}
              </Field>
            </FieldGroup>
          ...
          <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
            <Button
              ...
              onClick={() => (baseline === null ? undefined : setDraft(baseline))}
            >
            ...
            <Button
              type="submit"
              disabled={isLoading || isSaving || !isDirty || customDimensionsInvalid}
            >
        ...
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          <SummaryTile
            label="Capture"
            value={`Analytics capture is ${settingsDraft.enabled ? "enabled" : "disabled"}`}
          />
          <SummaryTile label="Privacy" value={formatPrivacyMode(settingsDraft.privacy_mode)} />
          <SummaryTile
            label="Consent"
            value={settingsDraft.consent_required ? "Required" : "Not required"}
          />
          <SummaryTile
            label="Journey sampling"
            value={`${settingsDraft.journey_sample_rate * 100}%`}
          />
          <SummaryTile label="Raw retention" value={`${settingsDraft.raw_retention_days} days`} />
          <SummaryTile
            label="Hourly retention"
            value={`${settingsDraft.hourly_retention_days} days`}
          />
          <SummaryTile
            label="Aggregate retention"
            value={`${settingsDraft.aggregate_retention_months} months`}
          />
        </div>
      ) : null}
      {hasLoadedSettings && settingsDraft.analytics_available ? (
  ...
  return (
    <Field orientation="horizontal" className="items-center justify-between gap-4">
      <div className="flex flex-1 flex-col gap-1">
        <FieldLabel id={`${input.id}-label`} htmlFor={input.id}>
          {input.label}
        </FieldLabel>
